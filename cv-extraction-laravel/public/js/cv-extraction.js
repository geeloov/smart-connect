document.addEventListener('DOMContentLoaded', function() {
    ...
    tabFormatted.addEventListener('click', function() {
        ...
        tabFormatted.classList.add('border-[#B9FF66]', 'text-dark');
        tabFormatted.classList.remove('border-transparent', 'text-dark', 'opacity-60');
        tabRaw.classList.add('border-transparent', 'text-dark', 'opacity-60');
        tabRaw.classList.remove('border-[#B9FF66]', 'text-dark');
    });

    tabRaw.addEventListener('click', function() {
        formattedView.classList.add('hidden');
        rawJson.classList.remove('hidden');
        tabRaw.classList.add('border-[#B9FF66]', 'text-dark');
        tabRaw.classList.remove('border-transparent', 'text-dark', 'opacity-60');
        tabFormatted.classList.add('border-transparent', 'text-dark', 'opacity-60');
        tabFormatted.classList.remove('border-[#B9FF66]', 'text-dark');
    });
    ...
    uploadForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('/cv-extraction/process', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': token
