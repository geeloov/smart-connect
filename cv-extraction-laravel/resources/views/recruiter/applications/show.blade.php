...
<div class="min-h-screen bg-gradient-to-b from-gray-50 to-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <!-- Header Card -->
        <div class="mb-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-white rounded-2xl shadow-lg border border-indigo-100 p-6">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-4 mb-2">
                        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 truncate">{{ $jobApplication->jobPosition->title }}</h1>
                        <span class="px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap bg-indigo-100 text-indigo-800">{{ $jobApplication->jobPosition->company_name ?? 'Your Company' }}</span>
                    </div>
                    <div class="flex flex-wrap gap-2 text-sm text-gray-500 mb-2">
                        ...
                        <span>({{ $jobApplication->created_at->diffForHumans() }})</span>
                    </div>
                    <div class="flex items-center gap-2 mt-2">
                        <span class="inline-block px-3 py-1 rounded-lg text-sm font-medium border
                            @if($jobApplication->status == 'pending') bg-yellow-100 text-yellow-800 border-yellow-200
                            @elseif($jobApplication->status == 'in_review') bg-blue-100 text-blue-800 border-blue-200
                            @elseif($jobApplication->status == 'accepted') bg-green-100 text-green-800 border-green-200
                            @elseif($jobApplication->status == 'rejected') bg-red-100 text-red-800 border-red-200
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div class="md:col-span-2">
                <div class="bg-gradient-to-br from-white to-blue-50 rounded-xl shadow-sm border border-blue-100 p-6 mb-8">
                    <div class="flex items-center gap-4 mb-4">
                        <div class="h-14 w-14 rounded-full flex items-center justify-center text-2xl font-bold text-white bg-blue-400">
                            {{ strtoupper(substr($jobApplication->jobSeeker->name, 0, 1)) }}
                        </div>
                        <div>
                            <h2 class="text-xl font-bold text-gray-900">{{ $jobApplication->jobSeeker->name }}</h2>
                            <p class="text-sm text-gray-600">{{ $jobApplication->jobSeeker->email }}</p>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-4 text-sm text-gray-500 mb-4">
                        <span>Applied: {{ $jobApplication->created_at->format('M d, Y') }}</span>
                        <span>Status: <span class="font-semibold text-gray-700">{{ ucfirst(str_replace('_', ' ', $jobApplication->status)) }}</span></span>
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="bg-white rounded-lg border border-gray-100 p-4">
                            ...
                            <div class="prose max-w-none text-gray-700">{!! nl2br(e($jobApplication->cover_letter)) !!}</div>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <form action="{{ route('recruiter.applications.update-status', $jobApplication) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
                    @csrf
                    @method('PATCH')
                ...
                <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
            ...
            @if($jobApplication->compatibility_score)
            <div class="bg-gradient-to-br from-white to-green-50 rounded-xl shadow-sm border border-green-100 p-6 flex flex-col items-center justify-center">
                ...
                <p class="text-gray-500 text-sm mt-1 text-center">Based on skills, experience, and education requirements</p>
            </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8 border border-blue-100">
                                    ...
                                    <div class="flex flex-col md:flex-row md:justify-between md:items-start">
                                        ...
                                        @if(isset($cert['date']) || isset($cert['year']) || isset($cert['issued_date']))
                                            <div class="mt-2 md:mt-0 bg-white px-3 py-1 rounded-full text-xs text-gray-600 font-medium border border-teal-100 inline-flex items-center shadow-sm">
                                                <svg class="h-4 w-4 mr-1 text-teal-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                                </svg>
